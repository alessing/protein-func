...
def save_hdf5(filename, protein_funcs, parser, D):
    ...
    if len(protein_data):
        structure = parser.get_structure(
            uniprot_id, f"{RAW_DATA}/{ALPHA_FOLD_DIR}/{filename}"
        )
        ...
        edge_index_tensor = torch.nonzero(
            torch.triu(adj_matrix)
        )  # Mask out lower triangle, and get i and j of edges
        ...
        edge_feats = np.vstack(
            (atom_type[edge_index], edge_dists)
        )  # row 1 is atom type 1, row 2 is atom type 2, row 3 is dist
        del distances
        del edge_dists
        ...
        with h5py.File(
            f"{PROCESSED_DATA}/hdf5_files_d_{D}/{uniprot_id}.hdf5", "w"
        ) as f:
            ...
            f.create_dataset(
                "adj_feats", data=adj_node_feats.cpu().numpy()
            )  # TODO: rename to adj_node_feats?
...
def main():
    ...
    arg_parser.add_argument("-d", type=int, default=10)
